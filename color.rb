...
1001: Must provide a url
1002: Too much info, buddy. I just need one url.
1003: Invalid url.
=end
...
class ColorPalette
    ...
    def check_www_url(auxurl)
        auxurl.sub! 'http://','http://www.' if !auxurl.start_with?("http://www.")
        auxurl = auxurl.chomp('/') if auxurl.end_with? '/' # chomp will making concatenating relative css files to url easier
        return auxurl
    end
    ...
    def get_stylesheet_urls(url)
        ...
        begin
            ...
            begin
                page = Nokogiri::HTML(open(check_www_url(url)).read)
            rescue
                handle_response(false,1003)
            end
        end
        ...
        page.css('style').each {|stylesheet|
            extract_css_color_palette(stylesheet.text)
        }
        page.css('[style]').each {|element|
            extract_css_color_palette(element['style'])
        }
        urls
    end
    ...
    def build_color_palette(urls)
        ...
        #    puts css_url
            begin
                css_source = Nokogiri::HTML(open(css_url)).text
            rescue
                # puts "Badly formed url:\n"+css_url
                next
            end
            css_source = Nokogiri::HTML(open(css_url)).text
            ...
            extract_css_color_palette( css_source )
        }
    end

    def extract_css_color_palette( css )
        color_array = css.scan(/color\s*:\s*(#[0-9A-Fa-f]{3,6}+)\s*|rgba?\s*\(\s*(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d{1,3})\s*|color\s*:\s*(white|aqua|black|blue|fuchsia|gray|green|lime|maroon|navy|olive|orange|purple)/)
    ...
    def print_color_palette_json
        handle_response(true,200, @color_map)
    end
...
def handle_response(success,code=200,*data)
	response = Hash.new
	response['success'] = success
	code = 1000 if !success && code==200
	response['code'] = code
	response['data'] = data
...
if ARGV.length == 0
	handle_response(false,1001)
elsif ARGV.length > 1
	handle_response(false,1002)
elsif ARGV.length == 1
	param_url = ARGV[0]
end
